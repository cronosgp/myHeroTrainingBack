...
public interface TreinoConjuntoRepository extends JpaRepository<TreinoConjunto, Integer> {
    ...
    List<TreinoConjunto> findByUsuarioId(int id);
    List<TreinoConjunto> findByContatoId(int id);

    @Query("SELECT u FROM TreinoConjunto u where u.idConvidado = ?1 AND u.idUsuario = ?2")
    Optional<TreinoConjunto> findByContatoIdAndUsuarioId(int convidadoid, int usuarioid);
